...
function spawnSnack(){
 ...
 el.addEventListener("click",()=>{if(el.classList.contains("hit"))return;gameHits++;$("gameHits").textContent=gameHits;el.classList.add("hit");setTimeout(()=>el.remove(),260);if(gameHits>=5)finishGame(true);});
...
function startGame(){
 ...
 const sp=setInterval(()=>{if(gameEnded){clearInterval(sp);return}spawnSnack();},500);spawnSnack();
}
async function finishGame(completed){
 ...
 if(!completed){gameIntro.hidden=false;gameIntro.innerHTML='<div class="lose-icon">⏰</div><h3>TIME! THE SNACK ESCAPED.</h3><p>You needed 5 hits. The boss is laughing.</p><button class="btn red" type="button" onclick="location.reload()">TRY AGAIN ↻</button>';return;}
 try{
